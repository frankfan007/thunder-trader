...
#pragma endregion
	...
	BEGIN_PARAMETER_BIND
		PARAMETER("BarTypeSecond", LB1_IntType, &BarTypeSecond)
		PARAMETER("m_intMinuendVolumePerCopy", LB1_IntType, &m_intMinuendVolumePerCopy)
		PARAMETER("m_intSubtrahendVolumePerCopy", LB1_IntType, &m_intSubtrahendVolumePerCopy)
		PARAMETER("m_intWindowsLen", LB1_IntType, &m_intWindowsLen)
		PARAMETER("m_dbWidth", LB1_DoubleType, &m_dbWidth)
		PARAMETER("m_dbInitMiddleWidth", LB1_DoubleType, &m_dbInitMiddleWidth)
		PARAMETER("m_intAlgorithmType", LB1_IntType, &m_intAlgorithmType)
...
#pragma region 参数
		...
		SHOW_STRING(to_simple_string(m_tdDayPeriodBegin).c_str())
		SHOW_STRING(to_simple_string(m_tdDayPeriodEnd).c_str())
...
#pragma region 参数
	...
	time_duration m_tdDayPeriodBegin=time_duration(1,2,0,0);//开仓，平仓
	time_duration m_tdDayPeriodEnd=time_duration(6,55,0,0);//平仓
	time_duration m_tdNightPeriodBegin=time_duration(13,2,0,0);//开仓，平仓
	time_duration m_tdNightPeriodEnd=time_duration(18,28,0,0);//平仓
...
void CMyStrategy::OnTick(TMarketDataIdType dataid, const CTick *pDepthMarketData)
{
	...
	if (m_boolStrategyOnOff&&m_deqHisteryBars.size() >= m_intWindowsLen)
	{
		...
		bool IsTimeOk =
			(m_ptimeGlobalCurrentTime.time_of_day() > m_tdDayPeriodBegin&&m_ptimeGlobalCurrentTime.time_of_day() < m_tdDayPeriodEnd)
			||
